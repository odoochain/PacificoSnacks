...
class Sales(models.Model):
    ...
    possible_arrival_date_destination = fields.Char('Posible fecha llegada a destino', compute='_possible_arrival_date_destination')
    ...
    def _possible_upload_date(self):
        ...
        if self.requested_delivery_date != False:
            ...
            if int(self.day_upload_week) > self.num_day:
               ...
               self.possible_upload_date = (day_upload - timedelta(days=resta)).strftime("%m-%d-%y-%A")
    ...
    @api.onchange('possible_upload_date')
    def _possible_arrival_date_destination(self):
        self.possible_arrival_date_destination = self.possible_upload_date